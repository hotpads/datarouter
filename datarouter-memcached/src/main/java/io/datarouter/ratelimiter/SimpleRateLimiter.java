...
public class SimpleRateLimiter{
	...
	/* Tests *****************************************************************/
	public static class IntegrationTests { //not technically integration tests, but slow.
		...
		public void testMutliThreaded() throws Exception{
			...
			/* The expected result if instructions were always instantaneous is
			 * one increment per time interval regardless of number of threads.
			 * Increasing the limit will make total==numTimeIntervals, but also
			 * make the test run more slowly.  If the rate limiter were allowing
			 * multiple threads to activate and increment incorrectly, total
			 * would be much larger than numTimeIntervals.
			 * */
