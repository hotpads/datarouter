...
public class UlidJsonSerializerTests{
	...
	public void testWrappedRoundTrip(){
		UlidAdapterTestDto input = new UlidAdapterTestDto(ULID);
		String json = GSON.toJson(input);
		String expected = """
				{"ulid":"01GFBY0JM7CN0E1J6WNAQHR17P"}""";
		Assert.assertEquals(json, expected);
		UlidAdapterTestDto output = GSON.fromJson(json, UlidAdapterTestDto.class);
		Assert.assertEquals(output.ulid, ULID);
	}

	private record UlidAdapterTestDto(Ulid ulid){
	}
