...
import io.datarouter.bytes.codec.floatcodec.RawFloatCodec;

public class RawFloatArrayCodec{

	public static final RawFloatArrayCodec INSTANCE = new RawFloatArrayCodec();

	private static final RawFloatCodec RAW_FLOAT_CODEC = RawFloatCodec.INSTANCE;
	private static final int ITEM_LENGTH = RAW_FLOAT_CODEC.length();
	...
	public int encode(float[] values, byte[] bytes, int offset){
		...
		for(int i = 0; i < values.length; ++i){
			RAW_FLOAT_CODEC.encode(values[i], bytes, cursor);
			cursor += ITEM_LENGTH;
		}
	...
	public float[] decode(byte[] bytes, int offset, int bytesLength){
		...
		for(int i = 0; i < resultLength; ++i){
			result[i] = RAW_FLOAT_CODEC.decode(bytes, cursor);
			cursor += ITEM_LENGTH;
		}
		return result;
	}
