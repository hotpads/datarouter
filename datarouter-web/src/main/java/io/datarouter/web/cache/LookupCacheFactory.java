...
public class LookupCacheFactory{
	...
	@Inject
	public LookupCacheFactory(
			@Named(DatarouterWebExecutorGuiceModule.POOL_lookupCache) ExecutorService executorService){
	...
	public static class LookupCache<K,V> implements LookupCacheGetters<K,V>{
		...
		public V getOrThrow(K key){
			return getInternal(key).orElseThrow(() -> exceptionFunction.apply(key));
		}
