...
public class ClusterSettingsHandler extends BaseHandler {
	...
	@Handler Mav edit() {
		...
		String prefixString = params.optional(P_prefix, null);
	...
	private Mav createOrUpdate(){
		ClusterSettingKey settingKey = parseClusterSettingKeyFromParams();
		String value = params.optional("value", null);
		Setting<?> setting = settingRoot.getSettingByName(settingKey.getName());
		//If setting is null, java does not know about that setting name yet, don't check the type
		if(setting == null || setting.isValid(value)){
			ClusterSetting clusterSetting = new ClusterSetting(settingKey, value);
			clusterSettingNodes.clusterSetting().put(clusterSetting, null);
		}
	...
	@Handler Mav browseSettings(){
		...
		for(Setting<?> setting : settingsList){
			...
			List<ClusterSetting> clusterSettings = clusterSettingNodes.clusterSetting()
					.stream(KeyRangeTool.forPrefix(settingKey), null).collect(Collectors.toList());
	...
	protected ClusterSettingKey parseClusterSettingKeyFromParams() {
		...
		String instance = params.optional(P_instance, "");
		String application = params.optional(P_application, "");
