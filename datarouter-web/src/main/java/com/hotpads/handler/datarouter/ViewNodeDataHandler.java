...
extends BaseHandler{
	...
	@Inject
	public ViewNodeDataHandler(DatarouterNodes nodes){
		this.nodes = nodes;
	}
...
//	}
	...
	public Mav browseData(){
		...
		if(!(node instanceof SortedStorageReader<?,?>)){
			return new MessageMav("Cannot browse non-SortedStorageReader" + node.getClass().getSimpleName());
		}
		if(node.getFieldInfo().getSampleFielder() == null){
			return new MessageMav("Cannot browse node without Fielder");
		}
...
//			startAfterKey = (PK)ReflectionTool.create(node.getPrimaryKeyType());
	...
	public Mav getWhere(){
		Mav mav = preHandle();
		if(!(node instanceof JdbcReaderNode<?,?,?>)){
		...
		List<D> databeans = node.getRouter().run(new GetWhereTxn((JdbcReaderNode)node, tableName, startAfterKey, where,
				config));
		addDatabeansToMav(mav, databeans);
		return mav;
	}
