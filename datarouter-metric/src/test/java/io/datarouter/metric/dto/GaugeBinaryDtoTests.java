...
import io.datarouter.metric.dto.GaugeBinaryDto.GaugeItemBinaryDto;
import io.datarouter.types.Ulid;

public class GaugeBinaryDtoTests{
	...
	public void testParse20230201(){
		var expected = new GaugeBinaryDto(
				"myService",
				"myServer",
...
//		HexBlockTool.print(expected.encodeIndexed());
		String hex = """
				...
				""";
		var actual = GaugeBinaryDto.decode(HexBlockTool.fromHexBlock(hex));
	...
	public void testCreateSizeDtosAll(){
		...
		Assert.assertEquals(GaugeBinaryDto.createSizedDtos(batch, numCounts).get(0).items.size(), numCounts);
		Assert.assertEquals(GaugeBinaryDto.createSizedDtos(batch, dtos.size()).get(0).items.size(), numCounts);
	}
