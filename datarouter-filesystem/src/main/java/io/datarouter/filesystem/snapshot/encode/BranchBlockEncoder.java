...
public interface BranchBlockEncoder extends DataBlockEncoder{
	...
	final Comparator<BranchBlockEncoder> LEVEL_AND_BLOCK_ID_COMPARATOR = Comparator
			.comparingInt(BranchBlockEncoder::level)
			.thenComparingInt(BranchBlockEncoder::blockId);

	void add(int blockId, long keyId, SnapshotEntry entry, int childBlockId);
