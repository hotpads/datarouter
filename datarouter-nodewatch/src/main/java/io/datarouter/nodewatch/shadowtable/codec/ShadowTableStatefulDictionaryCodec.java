...
/**
 * Store a list of column names in the blockfile header.
 * Map each column name to a VarInt (usually 1 byte).
 */
public class ShadowTableStatefulDictionaryCodec<
...
implements Codec<D,BlockfileRow>{
	...
	public ShadowTableStatefulDictionaryCodec(DatabeanFieldInfo<PK,D,F> fieldInfo, ColumnNameCodec columnNameCodec){
	...
	public BlockfileRow encode(D databean){
		...
		fielder.getNonKeyFields(databean).forEach(field -> {
			...
			if(valueBytes != null){
				...
				VarIntTool.encode(bytes, columnId);
				OutputStreamTool.write(bytes, valueBytes);
			}
		});
		byte[] valueBytes = bytes.toByteArray();
	...
	public D decode(BlockfileRow blockfileRow){
		// pk
		byte[] pkBytes = blockfileRow.copyOfKey();//TODO avoid copy
		...
		var reader = new ByteReader(blockfileRow.copyOfValue());//TODO avoid copy
