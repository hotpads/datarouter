...
public class SpannerLocalDateFieldCodec extends SpannerBaseFieldCodec<LocalDate,LocalDateField>{
	...
	public SpannerLocalDateFieldCodec(LocalDateField field){
		super(field);
	}
	...
	public Value getSpannerValue(){
		return SpannerValueTool.ofLocalDate(field.getValue());
	}

	@Override
	public Builder setKey(Builder key){
		LocalDate date = field.getValue();
		return key.append(Date.fromYearMonthDay(date.getYear(), date.getMonthValue(), date.getDayOfMonth()));
	}

	@Override
	public LocalDate getValueFromResultSet(ResultSet rs){
		Date value = rs.getDate(field.getKey().getColumnName());
