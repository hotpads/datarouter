...
public class HBaseHandler extends DatarouterRoutersAndClientsHandler{
	...
	protected Mav viewHBaseTableRegions(){
		...
		mav.setViewName(PATH_JSP_HBASE+"hbaseTableRegions.jsp");
		return mav;
	}
	/****************************************************/
	/********************** Update Handlers ***************/
	...
	protected Mav updateHBaseTableAttribute(){
		...
		initializeMav();
		mav.put("message-update", "HBase table attributes updated");
		return mav;
	...
	protected Mav updateHBaseColumnAttribute(){
		...
		initializeMav();
		mav.put("message-update", "HBase column attributes updated");
		return mav;
...
//		DRHServerList serverList = new DRHServerList(hbaseConfig);
//		
//		if(CollectionTool.doesNotContain(serverList.getServerNames(), serverName)){ throw new IllegalArgumentException(
...
//		}
		initializeMav();
		mav.put("message-update","HBase moved  several table regions to server " + "");
	...
	protected Mav compactHBaseTableRegions(){
		...
		initializeMav();
		mav.put("message-update","HBased compacted several table regions");
	...
	protected Mav compactAllHBaseTableRegions(){
		...
		mav.put("message-update","Submitted compact request for entire table " + tableName);
	...
	protected Mav majorCompactHBaseTableRegions(){
		...
		for(int i = 0; i < numRegions; ++i){
			...
			mav.put("message-update","Submitted majorCompact request region " + encodedRegionNameString);
		}
	...
	protected Mav majorCompactAllHBaseTableRegions(){
		...
		initializeMav();
		mav.put("message-update","Submitted majorCompact request for entire table " + tableName);
	...
	protected Mav flushHBaseTableRegions(){
		...
		initializeMav();
		mav.put("message-update","Flushed several table regions " );
	...
	protected Mav flushAllHBaseTableRegions(){
		...
		initializeMav();
		mav.put("message-update","Flushed all table regions " );
	...
	protected Mav mergeFollowingHBaseTableRegions(){
		...
		for(int i = 0; i < numRegions; ++i){
			...
			if(regionA == null){
				logger.warn(timer.add("couldn't find " + tableName + " region " + encodedRegionNameStrings.get(i)));
				continue;
			}
		...
		initializeMav();
		mav.put("message-update","Merged HBase table regions " );
	...
	private Mav initializeMav(){
		mav = new Mav("redirect:"+request.getContextPath()+"/datarouter/routers");
	...
	protected void initializeHBaseParameters(){
		...
		numRegions = CollectionTool.size(encodedRegionNameStrings);
		 ...
		 regionList = new DRHRegionList(hbaseClient, drhServerList, tableName, hbaseConfig, node,
				getBalancerStrategyForTable(tableName),drhCompactionInfo);
	}
	
	
	protected static String ACTION_countHBaseTableCells = "countHBaseTableCells", ACTION_copyHBaseTable = "copyHBaseTable",
