...
/**
 * Field is a mapping from a java field to the datastore. When using an RDBMS, Field defines the column name, datatype,
 * and metadata, but it is also used to transmit individual cells from java to the database. The getName() method refers
 * to the java field's name. This name is used to choose the default datastore column name, but the datastore column
 * name can be overridden with getColumnName().
 * 
