...
public class MasterSlaveNodeFactory{
	...
	newMasterSlaveMap(
		...
		if(masterClientName != null){
			master = (MapStorageNode<PK,D>)NodeFactory.create(masterClientName, databeanClass, fielderClass, router);
...
//			slaves.add(new HibernateNode<PK,D,F>(databeanClass, fielderClass, router, slaveClientName));
			MapStorageNode<PK,D> slaveNode = (MapStorageNode<PK,D>)NodeFactory.create(slaveClientName, databeanClass, 
					fielderClass, router);
