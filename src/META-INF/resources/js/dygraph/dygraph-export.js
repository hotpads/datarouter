...
/**
 * @license Copyright 2011 Juan Manuel Caicedo Carvajal (juan@cavorite.com)
 *          MIT-licensed (http://opensource.org/licenses/MIT)
 */

/**
 * @fileoverview This file contains additional features for dygraphs, which are
 *               not required but can be useful for certain use cases. Examples
 *               include exporting a dygraph as a PNG image.
 */

/**
 * Demo code for exporting a Dygraph object as an image. See:
 * http://cavorite.com/labs/js/dygraphs-export/
 */

define([ "dygraph/dygraph-combined" ],
        function() {
	        ...
	        // Texts displayed below the chart's x-axis and to the left of the
			// y-axis
	        ...
	        /**
			 * Tests whether the browser supports the canvas API and its methods
			 * for drawing text and exporting it as a data URL.
			 */
	        ...
	        /**
			 * Exports a dygraph object as a PNG image. dygraph: A Dygraph
			 * object img: An IMG DOM node userOptions: An object with the user
			 * specified options.
			 */
	        ...
	        /**
			 * Exports a dygraph into a single canvas object. Returns a canvas
			 * object that can be exported as a PNG. dygraph: A Dygraph object
	        ...
	        Dygraph.Export.drawPlot = function(canvas, dygraph, options) {
		        ...
		        Dygraph.Export
		                .putLabel(ctx, axesPlugin.ylabels_[i], options, options.labelFont, options.labelFontColor);
		        }

		        for (i = 0; i < axesPlugin.xlabels_.length; i++) {
		        Dygraph.Export
		                .putLabel(ctx, axesPlugin.xlabels_[i], options, options.labelFont, options.labelFontColor);
		        }
		        }
		        ...
		        Dygraph.Export.putLabel(ctx, labelsPlugin.title_div_, options, options.titleFont,
		                options.titleFontColor);

		        Dygraph.Export.putLabel(ctx, labelsPlugin.xlabel_div_, options, options.axisLabelFont,
		                options.axisLabelFontColor);

		        Dygraph.Export.putVerticalLabelY1(ctx, labelsPlugin.ylabel_div_, options, options.axisLabelFont,
		                options.axisLabelFontColor, "center");

		        Dygraph.Export.putVerticalLabelY2(ctx, labelsPlugin.y2label_div_, options, options.axisLabelFont,
		                options.axisLabelFontColor, "center");
		        }

		        for (i = 0; i < dygraph.layout_.annotations.length; i++) {
		        Dygraph.Export.putLabelAnn(ctx, dygraph.layout_.annotations[i], options, options.labelFont,
		                options.labelColor);
		        }
	        ...
	        Dygraph.Export.putLabel = function(ctx, divLabel, options, font, color) {
		        ...
		        // FIXME: Remove this 'magic' number needed to get the
				// line-height.
	        ...
	        Dygraph.Export.putVerticalLabelY1 = function(ctx, divLabel, options, font, color, textAlign) {
		        ...
		        // FIXME: The value of the 'left' property is frequently 0, used
				// the option.
		        if (!left)
	        ...
	        Dygraph.Export.drawLegend = function(canvas, dygraph, options) {
		        ...
		        // Drop the first element, which is the label for the time
				// dimension
		        ...
		        // TODO Replace the minus sign by a proper dash, although there
				// is a
			 ...
			 * Returns the the dictionary corresponding to the plugin for the
			 * argument. If the plugin is not found, it returns null.
			 */
