...
public class WebappInstanceHandler extends BaseHandler{
	...
	public Mav view(){
		...
		return webappInstanceTableService.buildMav(request, webappInstances, new WebappInstanceTableOptions(), List.of(
				...
				new WebappInstanceColumn<>("Servlet Container", WebappInstance::getServletContainerVersion)
						...
						.withAsBadge((inst, stats) -> !Objects.equals(inst.getServletContainerVersion(),
								stats.mostCommon)),
				new WebappInstanceColumn<WebappInstance>("Logs", $ -> i().withClasses("far", "fa-list-alt"))
						.withCellLinkBuilder(false, inst -> request.getContextPath()
								+ paths.datarouter.webappInstanceLog.toSlashedString()
								+ "?webappName=" + inst.getKey().getWebappName()
								+ "&serverName=" + inst.getKey().getServerName())));
	}
