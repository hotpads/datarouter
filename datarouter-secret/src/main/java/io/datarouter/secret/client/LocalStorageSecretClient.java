...
public class LocalStorageSecretClient extends BaseSecretClient{
	...
	private Properties readSecrets(){
		try{
			return PropertiesTool.parse(datarouterProperties.getConfigDirectory() + "/" + FILENAME);
		}catch(RuntimeException e){
			File file = new File(datarouterProperties.getConfigDirectory() + "/" + FILENAME);
			try{
				file.createNewFile();
	...
	private void writeSecrets(Properties properties){
		PropertiesTool.writeToFile(properties, datarouterProperties.getConfigDirectory() + "/" + FILENAME);
	}
