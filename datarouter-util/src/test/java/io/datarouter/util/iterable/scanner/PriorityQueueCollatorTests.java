...
public class PriorityQueueCollatorTests{
	...
	@BeforeMethod
	public void setup(){
		s1 = new ListBackedSortedScanner<>(Arrays.asList(1,3,8));
		s2 = new ListBackedSortedScanner<>(Arrays.asList(2,4,9));
		s3 = new ListBackedSortedScanner<>(Arrays.asList(5,6,7));
		s4 = new ListBackedSortedScanner<>(Arrays.asList(64,88,100));
	}

	@Test
	private void test(){
		PriorityQueueCollator<Integer> collator = new PriorityQueueCollator<>(Arrays.asList(s1, s2, s3, s4));
		Assert.assertEquals(new SingleUseScannerIterable<>(collator), Arrays.asList(1,2,3,4,5,6,7,8,9,64,88,100));
	}
	...
	private void testLimitedRestrict(){
		...
		Assert.assertEquals(new SingleUseScannerIterable<>(collator), Arrays.asList(1,2,3,4,5));
	}
