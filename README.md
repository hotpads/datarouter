...
is programmed against Datarouter’s interfaces and JDBC, Memcached, HBase, etc can be left as swappable dependencies.
...
trigger expensive queries and harder to accidentally cause large tablescans or joins that can affect the overall
application.
...
or plain text) for configuring things like web request handler mappings. An additional module, datarouter-webapp-utils 
includes many building blocks of a web app like log management, authentication, exception recording, counters, etc. 
...
Datarouter is an opinionated framework. This section covers the reasons behind some of the design choices. It also aims 
at explaining why the datarouter-storage interfaces seem very restrictive compared to the SQL language. For that 
purpose, it describes the internals of MySQL in a simplified way and compares with other Java ORMs.
...
many GB. Dirty pages are flushed to disk in the background. A row insertion causing the page to exceed 16 KB will result 
in the page being split in half causing ~8 KB of empty space at the end of each page. Fragmentation is rarely corrected, 
done by a manual and expensive `OPTIMIZE TABLE`. 

Queries served by the page cache can be answered relatively quickly. Queries needing many disk reads are much slower. 
...
The OS can combine many small fetches into fewer large fetches and do further pre-fetching. SSDs will last longer if you 
...
Let’s use a phone book as an example:
 ...
 Supported methods:
   ...
   - `SortedStorage`
     - `streamKeys`: return all firstName/lastNames from startKey to stopKey
     - `stream`: return all records from startKey to stopKey
     - `streamWithPrefix`: return records with:
...
Imagine executing this query manually on a phone book - you’d have to look at every row on every page. It’s not fun for 
...
as you get to 1 million people it becomes extremely inefficient.

Because the database only supports a limit on the number of matches, not on the number of skipped rows or total rows 
scanned, we can’t limit the cost of such a query. Because queries are executed in a transaction, this query may cause a 
...
the database it can be tricky to make sure they are kept up to date in an evolving application. Datarouter’s philosophy 
is to avoid making it easy to trigger an expensive query so you are forced to design the application to scale from the 
beginning.
...
session. Batch processing a large number of records can bring the session cache to a halt, meaning that the application 
becomes responsible for invalidating the cache during processing. Session cache management complicates the application 
...
performance, they need to add a caching layer like memcached or redis. These caching layers don’t speak SQL, forcing the 
developers to write custom code for each cache scenario or use more complicated ORM extensions or plugins. Later they 
need to move some large or frequently accessed tables to separate database machines. 

These slave databases, caches, and multiple databases break the original assumptions of the ORM. Hand-crafted SQL that 
joins two tables together must be rewritten into multiple queries so the join is done in the application. The links 
between databeans that were configured into the data model (usually via annotations) must be removed, and code that 
relied on them must be modified.
...
Datarouter advocates for assuming that all tables are on separate database servers to begin with. Joins are done at the 
