...
public class DatarouterGitMetrics extends BaseMetricRoot{
	...
	public static void incOp(DatarouterGitOp op){
		ROOT.op(op).count();
	}
	...
	private Op op(DatarouterGitOp op){
		return variable(Op::new, op.name);
	}

	private static class Op extends MetricNodeVariable<Op>{
		private Op(){
