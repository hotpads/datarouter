...
implements BlobClientNodeFactory, DatabeanClientNodeFactory, TallyClientNodeFactory{
	...
	@Inject
	private AwsMemcachedClientType awsMemcachedClientType;
	@Inject
	private ServiceName serviceName;
	@Inject
	private AwsMemcachedClientManager awsMemcachedClientManager;
	@Inject
	...
	public PhysicalBlobStorageNode createBlobNode(NodeParams<PathbeanKey,Pathbean,PathbeanFielder> nodeParams){
		var physicalBlobNode = new MemcachedBlobNode(
				nodeParams,
				awsMemcachedClientType,
				awsMemcachedClientManager.getLazyClient(nodeParams.getClientId()));
		return nodeAdapters.wrapBlobNode(physicalBlobNode);
	}
	...
	PhysicalNode<PK,D,F> createTallyNode(NodeParams<PK,D,F> nodeParams){
		var physicalTallyNode = new MemcachedTallyNode<>(
				nodeParams,
				awsMemcachedClientType,
				serviceName,
				awsMemcachedClientManager.getLazyClient(nodeParams.getClientId()));
		return nodeAdapters.wrapTallyNode(physicalTallyNode);
	}
