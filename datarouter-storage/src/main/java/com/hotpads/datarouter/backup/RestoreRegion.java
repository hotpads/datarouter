...
public abstract class RestoreRegion<PK extends PrimaryKey<PK>,D extends Databean<PK,D>> implements Callable<Void>{
	private static Logger logger = LoggerFactory.getLogger(RestoreRegion.class);
	...
	public RestoreRegion(Supplier<D> supplier, Router router, MapStorageNode<PK,D> node, Integer putBatchSize,
	...
	protected void importAndCloseInputStream(){
		try{
			...
			while(true){
				try{
					D databean = FieldSetTool.fieldSetFromByteStream(supplier, fieldByPrefixedName, is);
					toSave.add(databean);
					++numRecords;
...
//						logger.warn(timer);
					...
					if(toSave.size() > 0){//don't forget these
						node.putMulti(toSave, CONFIG_FAST_PUT_MULTI.setIgnoreNullFields(ignoreNullFields));
						logger.warn("imported " + DrNumberFormatter.addCommas(numRecords) + " from " + toSave.get(0)
								.getKey());
					}
