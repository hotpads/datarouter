...
public class ImportCounters{
	...
	public static class ImportCountersTests{
		...
		public void testGetKeyForImportAllAction(){
			...
			Assert.assertEquals("ListingImporter Import insert Listing", getKeyForImportAllAction(importerName,
					ACTION_insert));
			Assert.assertEquals("ListingImporter Import update Listing", getKeyForImportAllAction(importerName,
					ACTION_update));
			Assert.assertEquals("ListingImporter Import deactivate Listing", getKeyForImportAllAction(importerName,
					ACTION_deactivate));
			Assert.assertEquals("ListingImporter Import unmodified Listing hash", getKeyForImportAllAction(importerName,
					ACTION_unmodified, ACTION_TYPE_hash));
			Assert.assertEquals("ListingImporter Import unmodified Listing compare", getKeyForImportAllAction(
					importerName, ACTION_unmodified, ACTION_TYPE_compare));
			Assert.assertEquals("ListingImporter Import olderOrSame Listing providerVersion", getKeyForImportAllAction(
					importerName, ACTION_olderOrSame, ACTION_TYPE_providerVersion));
		}
		...
		public void testGetKeyForImportFeedAction(){
			...
			String importerName = "ListingImporter";
			Assert.assertEquals("ListingImporter ImportFeed FeedId insert Listing", getKeyForImportFeedAction(
					importerName, feedId, ACTION_insert));
			Assert.assertEquals("ListingImporter ImportFeed FeedId update Listing", getKeyForImportFeedAction(
					importerName, feedId, ACTION_update));
			Assert.assertEquals("ListingImporter ImportFeed FeedId deactivate Listing", getKeyForImportFeedAction(
					importerName, feedId, ACTION_deactivate));
			Assert.assertEquals("ListingImporter ImportFeed FeedId unmodified Listing hash", getKeyForImportFeedAction(
					importerName, feedId, ACTION_unmodified, ACTION_TYPE_hash));
		...
		public void testGetKeyForImportListingType(){
			...
			String importerName = "ListingImporter";
			Assert.assertEquals("ListingImporter ImportListingType rental insert Listing", getKeyForImportListingType(
					importerName, listingType, ACTION_insert));
			Assert.assertEquals("ListingImporter ImportListingType rental update Listing", getKeyForImportListingType(
					importerName, listingType, ACTION_update));
			...
			Assert.assertEquals("ListingImporter ImportListingType rental olderOrSame Listing providerVersion",
					getKeyForImportListingType(importerName, listingType, ACTION_olderOrSame,
							ACTION_TYPE_providerVersion));
		}

		@Test
		public void testGetKeyForRealTimeImportAllAction(){
			Assert.assertEquals("RealTimeImport markForUpdate Listing", getKeyForRealTimeImportAllAction(
					ACTION_markForUpdate));
			Assert.assertEquals("RealTimeImport receiveForUpdate Listing", getKeyForRealTimeImportAllAction(
					ACTION_receiveForUpdate));
		}

		@Test
		public void testGetKeyForRealTimeImportFeedAction(){
			Assert.assertEquals("RealTimeImportFeed markForUpdate Listing", getKeyForRealTimeImportFeedAction(
					ACTION_markForUpdate));
			Assert.assertEquals("RealTimeImportFeed receiveForUpdate Listing", getKeyForRealTimeImportFeedAction(
					ACTION_receiveForUpdate));
		}
	}
}
