...
public class TraceThreadGroup{
	...
	public void appendLeafThreadHtml(StringBuilder sb, TraceThread thread, int leafNum){
		...
		sb.append("<span style=\"" + spanStyle + "\">" + DrNumberFormatter.addCommas(thread.getRunningDuration())
				+ "ms</span>");
		sb.append("<span style=\"" + spanStyle + "\">" + DrNumberFormatter.addCommas(thread.getQueuedDuration())
				+ "ms</span>");
		sb.append("</div>");
	...
	public static class TraceThreadGroupSlownessComparator implements Comparator<TraceThreadGroup>{
		...
		public int compare(TraceThreadGroup groupA, TraceThreadGroup groupB){
			return DrComparableTool.nullFirstCompareTo(groupA.thread.getTotalDuration(), groupB.thread
					.getTotalDuration());
		}
	}
