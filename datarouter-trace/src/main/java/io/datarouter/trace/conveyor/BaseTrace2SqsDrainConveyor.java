...
public abstract class BaseTrace2SqsDrainConveyor
extends BaseGroupQueueConsumerConveyor<ConveyorMessageKey,ConveyorMessage>{
	...
	protected void processDatabeans(List<ConveyorMessage> databeans){
		Trace2BatchedBundleDto dto = Scanner.of(databeans)
				.map(ConveyorMessage::getMessage)
				.map(message -> gson.fromJson(message, Trace2BundleDto.class))
				.listTo(Trace2BatchedBundleDto::new);
		persistData(dto);
	}

	public abstract void persistData(Trace2BatchedBundleDto batchDto);
