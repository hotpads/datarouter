...
extends KinesisOp<PK,D,F,BlockingQueue<StreamRecord<PK,D>>>{
	...
	public KinesisStreamLatest<PK,D,F> withApplicationName(String applicationName){
	...
	protected final BlockingQueue<StreamRecord<PK,D>> run(){
		StreamSubscriber<PK,D,F> streamSubscriber = new StreamSubscriber<>(streamName, applicationName, regionName,
	...
	D extends Databean<PK,D>, F extends DatabeanFielder<PK,D>>{
		...
		public StreamSubscriber(String streamName, String applicationName, String regionName,
			...
			applicationName = applicationName + "_" + initialPositionInStream.name();
	...
	D extends Databean<PK,D>, F extends DatabeanFielder<PK,D>> implements IRecordProcessor {
		private static final Logger logger = LoggerFactory.getLogger(
				KinesisStreamLatest.AmazonKinesisApplicationSampleRecordProcessor.class);
        ...
        private void processSingleRecord(Record record) throws InterruptedException {
			...
			String json = new String(data.array(), Charset.forName("UTF-8"));
			D databean = codec.fromString(json, fielder, databeanSupplier);
