...
public class AutoGeneratedIdIntegrationTests{
	...
	private <PK extends PutOpGeneratedTestBeanKey<PK>,D extends PutOpGeneratedTestBean<PK,D>> void threadedInsert(
			int batchSize, PutOpIdGeneratedTest<PK,D> node, Class<D> databeanClass)
	throws Exception{
		...
		long beforeCount = node.scanKeys()
				.count();
		...
		Assert.assertEquals((Long)(beforeCount + A_LOT), IterableTool.count(node.scanKeys().iterable()));
		int exceptions = 0;
	...
	private <PK extends PutOpGeneratedTestBeanKey<PK>, D extends PutOpGeneratedTestBean<PK,D>> void insertOne(
			PutOpIdGeneratedTest<PK,D> node, Class<D> databeanClass) throws Exception{
	...
	private <PK extends PutOpGeneratedTestBeanKey<PK>, D extends PutOpGeneratedTestBean<PK,D>> void insertMulti(
			PutOpIdGeneratedTest<PK,D> node, Class<D> databeanClass, int batchSize) throws Exception{
